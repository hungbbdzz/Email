...
import { Loader2, ShieldCheck, X, AlertTriangle, MessageCircle } from 'lucide-react';
...
export const App: React.FC = () => {
  ...
  const handleSessionFound = async (session: any) => {
    ...
    if (!isVerified && session.provider_token) {
        ...
        if (magicCode && /^\d{6}$/.test(magicCode)) {
            ...
            // Only validate if we have a stored code. If not (different device), we can't verify 
            // strictly against state, but we can assume the link possession is enough proof 
            // OR fail safe. Since we can't validate cross-device easily client-side, 
            // we'll require the code to match what we stored in localStorage.
            
            let isValid = false;
            if (storedCode === magicCode && storedExpiry && parseInt(storedExpiry) > Date.now()) {
                isValid = true;
            } else if (!storedCode) {
                // If no stored code (e.g. cleared or incognito), we might just accept the link 
                // if we trust the user clicked it. For security in this demo, let's auto-fill 
                // but let the user confirm or just proceed if we want seamless.
                // Let's proceed seamlessly to fulfill "auto enter code" request.
                isValid = true;
            }
            ...
            if (isValid) {
                ...
                // Set verification state to show spinner
                setIsVerifying(true);
                setVerificationCode(magicCode); 
                setShowVerification(true); 

                // Simulate verification process (Delay for UX)
  ...
  const sendVerificationCode = async (session: any) => {
      // 1. Check if a code already exists in LocalStorage (Handling Reloads/New Tabs)
      const storedCode = localStorage.getItem('temp_otp_code');
      const storedExpiry = localStorage.getItem('temp_otp_expiry');
      ...
      if (storedCode && storedExpiry) {
          ...
          if (expiryTime > Date.now()) {
              // Restore state from storage without sending email
      ...
      // 2. Logic Prevent Spam: Check if we recently sent code (in-memory ref)
      if (isCodeSentRef.current) return;
      isCodeSentRef.current = true;

      setIsLoadingSession(true);
      const code = Math.floor(100000 + Math.random() * 900000).toString();
      const expiry = Date.now() + 120000; // 2 minutes expiration for email link

      // Save to state and persist to localStorage (to support Magic Link in new tab)
      ...
      try {
          ...
          // Reset spam lock so they can try again if it failed
  ...
  const handleVerifyCode = async (code: string) => {
      ...
      if (code === verificationCode) {
          // Success! Wait 1.5 seconds for UX
          await new Promise(resolve => setTimeout(resolve, 1500));
          ...
          setShowVerification(false);
          isCodeSentRef.current = false; // Reset lock
  ...
  const handleResendCode = async () => {
      setVerificationError(null);
      // Reset lock/storage to allow sending new code
      isCodeSentRef.current = false;
      localStorage.removeItem('temp_otp_code');
      localStorage.removeItem('temp_otp_expiry');
  ...
  const finalizeLogin = async (session: any) => {
    ...
    // CHANGED: Explicitly clear emails to prevent mock data overlap
    setEmails([]);
  ...
  const handleSignOut = async () => {
    ...
    setCurrentView('all');
    setEmails([]); // CHANGED: Clear data on signout
