...
const transformGmailToAppEmail = (gmailMessage: any): Email => {
  ...
  const initialLabel = classifyHeuristicInternal(
      decodedSubject, 
...
const CLASSIFICATION_KEYWORDS: Record<string, string[]> = {
  [EmailLabel.GAME]: ["steam", "valve", "riot", "genshin", "hoyoverse", "mihoyo", "paimon", "honkai", "battlenet", "epic games", "playstation", "xbox", "twitch", "discord", "primogem"],
  [EmailLabel.EDUCATION]: ["duolingo", "streak", "lesson", "learning", "coursera", "udemy", "edx", "homework", "assignment", "grade", "class", "lớp", "học tập", "quiz", "exam"],
  [EmailLabel.WORK]: ["meeting", "agenda", "report", "deadline", "task", "project", "dự án", "tài liệu", "document", "sếp", "hr", "contract", "invoice", "salary", "chấm công", "jira", "slack", "trello", "teams", "zoom", "github"],
  [EmailLabel.PROMOTION]: ["shopee", "lazada", "tiki", "tiktok shop", "grab", "be", "booking.com", "traveloka", "agoda", "sale", "discount", "voucher", "coupon", "giảm giá", "khuyến mãi", "hoàn xu", "flash sale"],
  [EmailLabel.SPAM]: ["xổ số", "bet", "casino", "tài xỉu", "vay vốn", "trúng thưởng", "congratulations", "you won", "claim now", "click here", "việc nhẹ lương cao"],
  [EmailLabel.PERSONAL]: ["facebook", "instagram", "twitter", "messenger", "zalo", "viber", "telegram", "whatsapp", "birthday", "party", "wedding", "hẹn hò", "ba mẹ", "người yêu", "coffee"],
  [EmailLabel.PHISHING]: ["security alert", "verify account", "account locked", "suspended", "password reset", "banking", "unusual activity", "action required", "cảnh báo bảo mật", "khóa tài khoản"]
};

const classifyHeuristicInternal = (subject: string, sender: string, preview: string): { label: EmailLabel, score: number } => {
    let label = EmailLabel.WORK; 
    let score = 0.3;
...
export const classifyAndSyncEmails = async (emails: Email[], userId: string, addModelLog: (action: string, details: string, status: string) => void) => {
  ...
  addModelLog('Heuristic Classification', `Processed ${emails.length} emails locally`, 'Success');
