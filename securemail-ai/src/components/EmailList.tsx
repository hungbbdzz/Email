...
export const EmailList: React.FC<EmailListProps> = ({ 
  ...
  const handleBatchSelect = (type: SelectionType) => {
    ...
    if (type === 'all') processedEmails.forEach(e => newSet.add(e.id));
    else if (type === 'read') processedEmails.filter(e => e.isRead).forEach(e => newSet.add(e.id));
    else if (type === 'unread') processedEmails.filter(e => !e.isRead).forEach(e => newSet.add(e.id));
    else if (type === 'starred') processedEmails.filter(e => e.isStarred).forEach(e => newSet.add(e.id));
    else if (type === 'unstarred') processedEmails.filter(e => !e.isStarred).forEach(e => newSet.add(e.id));
    setCheckedEmailIds(newSet);
    setIsSelectMenuOpen(false);
  };
  ...
  const getSnoozeOptions = () => {
    ...
    // Later Today: 6 PM today (if before 5pm, otherwise skip)
    ...
    // Next Week: Next Monday 8 AM
    const nextWeek = new Date();
    nextWeek.setDate(nextWeek.getDate() + (1 + 7 - nextWeek.getDay()) % 7 || 7); // Calculate next monday
    ...
    options.push({ 
        label: 'Tomorrow', 
        time: tomorrow.toLocaleDateString('en-US', { weekday: 'short' }) + ', 8:00 AM', 
        date: tomorrow 
    });
    
    options.push({ 
        label: 'Next week', 
        time: nextWeek.toLocaleDateString('en-US', { weekday: 'short' }) + ', 8:00 AM', 
        date: nextWeek 
    });
  ...
  const isAllSelected = processedEmails.length > 0 && checkedEmailIds.size === processedEmails.length;
  const isPartiallySelected = checkedEmailIds.size > 0 && checkedEmailIds.size < processedEmails.length;
                ...
                <div className="relative" ref={filterRef}>
                  ...
                  {isFilterOpen && (
                    <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-20 py-2">
                      ...
                      {Object.values(EmailLabel).map(label => (
                         <button key={label} onClick={() => { setActiveFilter(label); setIsFilterOpen(false); }} className="w-full text-left px-4 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-700 flex justify-between dark:text-slate-200">
                          <span>{label}</span>
        ...
        {processedEmails.length === 0 ? (
          ...
          <div className="flex flex-col relative">
            {processedEmails.map((email) => {
              const isSelected = checkedEmailIds.has(email.id);
              ...
              return (
                ...
                >
                  ...
                  {/* Avatar & Selection (Gmail Style: Hover to reveal checkbox) */}
                  <div className="flex-shrink-0 relative w-10 h-10 group/avatar pl-2 md:pl-0">
                      
                      {/* Avatar Image - Hidden if selected OR if row hovered (on Desktop) */}
                      <img 
                        src={email.avatar} 
                        alt="" 
                      ...
                      {/* Checkbox Overlay - Visible if selected OR if row hovered */}
                      <div 
                        ...
                        onClick={(e) => handleCheck(e, email.id)}
                      >
                  ...
                  <div className="flex-1 min-w-0 flex items-center gap-4">
                      ...
                      {/* Sender - WIDTH REDUCED TO 2/3 (w-20 md:w-32) */}
                      <div className="w-20 md:w-32 flex-shrink-0 flex items-center gap-2 overflow-hidden">
                          <span className={`text-sm truncate ${!email.isRead ? 'font-bold text-slate-900 dark:text-white' : 'font-medium text-slate-700 dark:text-slate-300'}`}>
                              {email.senderName}
                          </span>
                          {email.label === EmailLabel.PHISHING && <div className="w-2 h-2 rounded-full bg-red-500 flex-shrink-0" title="Phishing Alert"></div>}
                      </div>

                      {/* Subject & Snippet - Takes remaining space */}
                      ...
                      {/* Date & Labels (Desktop) - Aligned to right */}
                      <div className="hidden md:flex flex-shrink-0 justify-end items-center gap-3 w-auto min-w-[80px]">
                          {/* Star Visual Indicator (Only if starred, not interactive here) */}
                          {email.isStarred && (
